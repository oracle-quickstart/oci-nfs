...
value = <<END
        ...
        NFSv4 mount for root NFS shared folder(denoted as / instead of /mnt/nfsshare/exports): sudo mount -t nfs -o vers=4,defaults,noatime,bg,timeo=100,ac,actimeo=120,nocto,rsize=1048576,wsize=1048576,nolock,local_lock=none,proto=tcp,sec=sys,_netdev  ${local.nfs_server_ip}:/mnt/nfsshare/exports  /mnt/nfsv4

        NFSv4 mount for NFS shared folder(denoted as /export1 instead of /mnt/nfsshare/exports/export1): sudo mount -t nfs -o vers=4,defaults,noatime,bg,timeo=100,ac,actimeo=120,nocto,rsize=1048576,wsize=1048576,nolock,local_lock=none,proto=tcp,sec=sys,_netdev  ${local.nfs_server_ip}:/mnt/nfsshare/exports/export1  /mnt/nfsv4_export1
